...
import StockFilter from '../Common/StockFilter1';
...
import { BLACKTokenABI, BLACKTokenAddress, LPTokenABI, LPTokenAddress, StakingABI, StakingBLACKAddress, StakingBlackABI, stakingLPAddress } from '../../abi/abi';
...
const MainPoolBox = ({ Item, balanceOfTokens }) => {
...
console.log("notregiterd",notregistered)
    ...
    useEffect(()=>{displayValueCalculation()},[address])
    ...
    const displayValueCalculation = async() =>{
      if(localStorage.getItem("walletAddress") === null || localStorage.getItem("walletAddress") === undefined || localStorage.getItem("walletAddress") === ''){                
          ...
          //new codes
          const BlackContract = new ethers.Contract(BLACKTokenAddress, BLACKTokenABI, provider);
          const BlackLPStaking = new ethers.Contract(StakingBLACKAddress, StakingBlackABI, provider);
          console.log("totalLpLiquidity")
          
          let totalLpLiquidity =  ethers.utils.formatUnits(await BlackContract.balanceOf(StakingBLACKAddress),0);
          setTotalStakedAmount(totalLpLiquidity);

          const LpContract = new ethers.Contract(LPTokenAddress, LPTokenABI, provider);
          let LpContractBalance =  ethers.utils.formatUnits(await LpContract.balanceOf(stakingLPAddress),0);
          ...
          let allowance =  ethers.utils.formatUnits(await BlackContract.allowance(localStorage.getItem("walletAddress"),StakingBLACKAddress),0);
          ...
          recordUserVisits(address, "StakeMe");
    ...
    const StockFilterData1 = [
      {
        StakedName:"Staked",
        StakableNameNum: mystaked?(mystaked/1e9) :"0" ,
        Stake_All_btns_text:"",
        All_btns_text:"Click to Stake",
        Filter_id : "Stake",
        rewardtoclaim: minReward?minReward:0,
        unstakeTime: unstakeRemainingTime?unstakeRemainingTime:0,
        unstake: lock
    },
      {
          StakedName:"Earned",
          StakableNameNum:Myreward?(Myreward/1e9) :"0",
          Stake_All_btns_text:"",
          All_btns_text:"Claim",
          Filter_id : "Claim",
          rewardtoclaim: minReward?minReward:0,
    ...
    const maxval = async() =>{
        ...
        // }
        // else{
            setamount(Math.abs(myBalance)/1e9)
        // }
    }
    ...
    const maxval1 = async() =>{
      ...
      // }
      // else{
          setamount(Math.abs(mystaked)/1e9)
      // }
  }
...
const deposit = async() =>{
  ...
  try{
    ...
    const dimeUSDCLPstakingContract = new ethers.Contract(StakingBLACKAddress, StakingBlackABI, signer);
    ...
    const mintTx = await dimeUSDCLPstakingContract.deposit(amount*1e9);
    await mintTx.wait();
    ...
    resetInputs();
    await sleep(2000);
    await createTxn("ME",mintTx.hash,"Stake ME",StakingBLACKAddress,address);
    setLoader(false)
    // await sleep(1600);
    ...
    toast.error("Staking is not succeed",`${(error?.reason).toString()}`);
...
const unstake = async() =>{
  ...
  try{
    ...
    // const val1 =  ethers.utils.parseUnits(val11, 18);;
    // Send the transaction and wait for it to be mined
    const mintTx = await dimeUSDCLPstakingContract.withdraw(amount*1e9);
    await mintTx.wait();
    console.log("minttx",mintTx.hash);
    // toast.success(` "Successfully Minted JUSD", ${(mintTx.hash)} `)
    let id = "https://sepolia.basescan.org/tx/" + mintTx.hash;
    ...
    await sleep(2000);
    setLoader(false)
    await createTxn("ME",mintTx.hash,"UnStake ME",StakingBLACKAddress,address);
    // await sleep(1600);
    // window.location.reload();
    setEnableUseEffect(false);
    setOpens(false)
}catch(error){
    toast.error("UnStaking is not succeed",`${(error?.reason).toString()}`);
    console.log("error",error)
    setLoader(false)
}
}
...
const approve = async() =>{
  ...
  try{
    ...
    const signer =  ethersProvider.getSigner()
      ...
      // Create contract instance with the correct order of arguments
      const lptoken = new ethers.Contract(BLACKTokenAddress, BLACKTokenABI, signer);

      const mintTx = await lptoken.approve(StakingBLACKAddress,BigInt(10000000000*1e9));
                    ...
                    <div className='Withdraw_main_row_row'>
                        <div className='coverage_ratio_block'>
                            ...
                            <div className='mer_dai_icons'>
                                            ...
                                            {/* <img src={avaxLogo} width="40" height={"40"} alt="mer_dai_icon" /> */}
                                            <p>ME</p>
                                        </div>
                        ...
                        <div className='deposits_block'>
                            ...
                            <h5 className='mer_text_h5'> {TotalStakedAmount?formatNumber(Math.abs(TotalStakedAmount/1e9).toFixed(9)):"0.0"}</h5>
                        ...
                        <div className='deposits_block'>
                            ...
                            <h5 className='mer_text_h5'>{mystaked?formatNumber(Math.abs(mystaked/1e9).toFixed(9)):"0.0"}</h5>
                ...
                <div className='reward_main_block'>
                    <div className='reward_row'>
                        <div className='reward_logo_sec'>
                            ...
                            <p className='ratio_common_p'>Base APR: <span>{TotalStakedAmount>0 ? parseFloat((((16534391534*525600)/TotalStakedAmount)*100)/1e9).toFixed(2) :'0'}%</span></p>
                        </div>
                        <div>
                        ...
                        <div>
                            <p className='ratio_common_p'>My Boosted APR: <span> {mystaked>0 ? parseFloat((((16534391534*525600)/mystaked)*100)/1e9).toFixed(2) :'0'}%</span></p>
                        </div>
                        <div>
                ...
                <div className='swap_tokens_list'>
                  ...
                  <div className='Confirm_Stack_main'>
                    <div className='Confirm_Stack_P_row'>
                      ...
                      <span className='Market_text_p'>Stakable: {myBalance?parseFloat(Math.abs(myBalance/1e9)).toFixed(4):"0.0"} ME</span>
                    </div>
                    <div className='Confirm_Stack_box'>
                    <input type="text" placeholder='0.00' onChange={event => changeInputValue(event.target.value)} value={amount?(amount) : ''}/>
                    ...
                    <div className='Confirm_Swap_pop_btn'>
                      ...
                      {parseFloat(allowan) >= parseFloat(amount*1e9)?(<>
              ...
              <div className='swap_tokens_pop_main'>
                ...
                <div className='swap_tokens_list'>
                  ...
                  <div className='Confirm_Stack_main'>
                    <div className='Confirm_Stack_P_row'>
                      {/* <p className='Market_text_p'>Your LP balance:  {lpbalance?parseFloat(Math.abs(lpbalance/100000000)).toFixed(4):"0.0"} USDCLP</p> */}
                      <span className='Market_text_p'>Staked: {mystaked?parseFloat(Math.abs(mystaked/1e9)).toFixed(4):"0.0"} ME</span>
                    </div>
                    <div className='Confirm_Stack_box'>
                    <input type="text" placeholder='0.00' onChange={event => changeInputValue(event.target.value)} value={amount?(amount) : ''}/>
                      <div className="max_btn">
                ...
                <div className='swap_tokens_list'>
                  ...
                  <div className='Confirm_Stack_main'>
                    ...
                    <ul className='Confirm_Stack_lists'>
                      <li>
                        <p className='Market_text_p'>Your ME balance: </p>
                        <span className='Market_text_p'> {lpbalance?parseFloat(Math.abs(lpbalance/100000000)).toFixed(4):"0.0"} </span>
                      </li>
                      {/* <li>
                        <p className='Market_text_p'>Token Stake</p>
                        <span className='Market_text_p'>0.00 Mercury</span>
